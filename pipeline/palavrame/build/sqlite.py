...
def _write_entries(
    ...
    # Indexado pela GRAFIA EXATA. Indexar pelo normalizado — como já foi —
    # fazia `Internet` e `internet` partilharem a chave: o segundo id
    # sobrescrevia o primeiro e as aceções de ambos caíam no mesmo lema,
    # deixando o outro vazio na DB. O mapa normalizado existe à parte, só
    # como recurso para resolver alvos de relações da wordnet.
    ...
    for i, entry in enumerate(entries, start=1):
        ...
        lemma_ids[entry.lemma] = i
        norm_ids.setdefault(entry.normalized, i)
        counts["lemmas"] += 1

    sense_id = 0
    example_id = 0
    for entry in entries:
        lemma_id = lemma_ids[entry.lemma]
    ...
    for entry in entries:
        ...
        for relation in entry.relations:
            ...
            # Grafia exata primeiro; se a wordnet escrever o alvo de outra
            # maneira, cai-se no normalizado (primeiro lema desse grupo).
            target_id = lemma_ids.get(relation.target) or norm_ids.get(
    ...
    _limpar_fonte_nao_usada(conn, unknown_id)
...
def _limpar_fonte_nao_usada(conn: sqlite3.Connection, unknown_id: int) -> None:
    """Apaga a rede de segurança quando ela não foi precisa — o caso normal.

    A linha "Fonte não declarada" tem de existir **durante** a escrita, para
    servir de destino a qualquer conteúdo que chegue sem fonte registada. Mas
    ficava na tabela mesmo sem nada apontar para ela, e o ecrã "Fontes e
    licenças" da app lista tudo o que lá está — aparecia ao utilizador uma
    fonte chamada "Fonte não declarada", com licença "DESCONHECIDA", que não
    contribuiu com uma única palavra.

    Não se pode filtrar pelo ecrã em vez disto: o Hunspell e o PAPEL também
    não têm aceções nem exemplos — dão formas e relações — e têm de continuar
    a aparecer, porque as suas licenças exigem atribuição.
    """
    em_uso = conn.execute(
        "SELECT EXISTS(SELECT 1 FROM senses WHERE source_id = ?)"
        " OR EXISTS(SELECT 1 FROM examples WHERE source_id = ?)",
        (unknown_id, unknown_id),
    ).fetchone()[0]
    if not em_uso:
        conn.execute("DELETE FROM sources WHERE id = ?", (unknown_id,))

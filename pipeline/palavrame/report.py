...
class BuildReport:
    ...
    def to_markdown(self) -> str:
        ...
        if total == 0:
            ...
            lines.append("")
            contagens = self.conflicts.get("contagens") or {}
            for key in ("pos_disagreements", "rejected_lemmas",
                        "duplicate_senses", "orphan_examples"):
                items = self.conflicts.get(key) or []
                if not items:
                    continue
                # `contagens` tem o número real; `items` é só a amostra que o
                # ConflictReport guardou (ver MAX_CONFLITOS_REGISTADOS).
                quantos = contagens.get(key, len(items))
                lines += [f"### {key} ({quantos})", ""]
                for item in items[:20]:
                    lines.append(f"- `{json.dumps(item, ensure_ascii=False)}`")
                if quantos > 20:
                    lines.append(f"- … mais {quantos - 20}")
                lines.append("")

...
def validate_database(
    ...
    *,
    ...
    try:
        ...
        _check_curadoria(conn, report)
        _check_preenchimento(conn, report)
...
CURADORIA = "Palavra-me"
WORDNET = "Wordnet do português (PULO / OpenWordNet-PT)"


def _check_preenchimento(conn: sqlite3.Connection, report: ValidationReport) -> None:
    """Quantas palavras dependem só das glosas traduzidas do wordnet.

    Estas são as entradas mais frágeis da base: a definição é tradução
    automática do inglês de Princeton e ninguém a reviu. O número interessa
    porque é a fila de espera da curadoria — e porque, se um dia crescer sem
    explicação, é sinal de que uma fonte melhor deixou de estar a entrar.
    """
    so_wordnet = conn.execute(
        "SELECT COUNT(*) FROM lemmas l WHERE EXISTS ("
        "  SELECT 1 FROM senses s JOIN sources src ON src.id = s.source_id"
        "  WHERE s.lemma_id = l.id AND src.name = ?)"
        " AND NOT EXISTS ("
        "  SELECT 1 FROM senses o JOIN sources so ON so.id = o.source_id"
        "  WHERE o.lemma_id = l.id AND so.name <> ?)",
        (WORDNET, WORDNET),
    ).fetchone()[0]
    if so_wordnet:
        report.add(
            "só com glosa do wordnet", "info",
            f"{so_wordnet} palavras cuja única definição é tradução automática "
            "— candidatas naturais a curadoria manual",
        )


def _check_curadoria(conn: sqlite3.Connection, report: ValidationReport) -> None:
    """A curadoria manual só deve preencher lacunas.

    Regra dada pelo Jorge quando aprovou a fonte: escreve-se à mão *só* o que
    não está disponível em mais nenhuma fonte aberta. Uma palavra curada que
    entretanto ganhou definição publicada — porque o Wikcionário cresceu, ou
    porque uma fonte nova entrou — deixou de precisar da linha, e é isso que
    este aviso serve para dizer. Nunca bloqueia: a definição curada continua
    válida e fica em segundo lugar, mas a linha pode sair do CSV.
    """
    curadas = conn.execute(
        "SELECT COUNT(*) FROM senses s JOIN sources src ON src.id = s.source_id"
        " WHERE src.name = ?", (CURADORIA,)
    ).fetchone()[0]
    if not curadas:
        return
    report.add("curadoria", "info", f"{curadas} aceções escritas à mão")

    redundantes = [
        row["lemma"]
        for row in conn.execute(
            "SELECT DISTINCT l.lemma FROM lemmas l"
            " JOIN senses c ON c.lemma_id = l.id"
            " JOIN sources sc ON sc.id = c.source_id AND sc.name = ?"
            " WHERE EXISTS ("
            "   SELECT 1 FROM senses o JOIN sources so ON so.id = o.source_id"
            "   WHERE o.lemma_id = l.id AND so.name <> ?)"
            " ORDER BY l.lemma",
    ...
    if redundantes:
        report.add(
            "curadoria redundante", "aviso",
            f"{len(redundantes)} palavras curadas já têm definição de fonte "
            "aberta — as linhas podem sair de seeds/curadoria.csv",
            ", ".join(redundantes[:12]) + (" …" if len(redundantes) > 12 else ""),
        )
